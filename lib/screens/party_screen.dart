...
class _PartyScreenState extends State<PartyScreen> {
      ...
      builder: (context) => Column(
        ...
        children: [
          ...
          ListTile(
            ...
            title: Text(TranslationService.translate('edit', lang)),
            onTap: () {
          ...
          ListTile(
            ...
            title: Text(TranslationService.translate('delete', lang),
  ...
  Future<void> _editParty(Party party) async {
    ...
    final emailController = TextEditingController(text: party.email);
    final balanceController =
        TextEditingController(text: party.openingBalance.toString());
    ...
    await showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          title: Text(TranslationService.translate('edit_party', lang)),
            ...
            ElevatedButton(
              onPressed: () async {
                if (nameController.text.isNotEmpty) {
                  ...
                  if (index != -1) {
                    setState(() => _parties[index] = updatedParty);
                    await StorageService.saveParties(widget.activeFY, _parties);
                    _loadData();
                  }
  ...
  Future<void> _deleteParty(Party party) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text(TranslationService.translate('delete', lang)),
        content: Text(TranslationService.translate('confirm_delete', lang)),
        actions: [
          TextButton(
          ...
          TextButton(
              ...
              child: Text(TranslationService.translate('delete', lang),
                  style: const TextStyle(color: Colors.red))),
        ],
      ),
    );
                    ...
                    return TweenAnimationBuilder<double>(
                      ...
                      child: Card(
                        ...
                        child: ListTile(
                          ...
                          onLongPress: () => _showPartyOptions(party),
                        ),
                      ),
                    );
                  },
...
class _PartyLedgerScreenState extends State<PartyLedgerScreen> {
  ...
  Future<void> _loadLedger() async {
    ...
    double balance = widget.party.openingBalance * (widget.party.type == BalanceType.dr ? 1 : -1);
    List<double> balances = [balance];
  ...
  Future<void> _editEntry(PartyTransaction entry) async {
    final lang = languageNotifier.value;
    final particularController = TextEditingController(text: entry.particular);
    final amountController = TextEditingController(
        text: (entry.debit > 0 ? entry.debit : entry.credit).toString());
    String selectedEntryType = entry.debit > 0 ? 'Debit' : 'Credit';
    ...
    if (dateTypeNotifier.value != 'AD') {
      try {
        ...
        selectedNepaliDate = ndp.NepaliDateTime.now();
      }
    }
    ...
    await showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          title: Text(TranslationService.translate('edit_ledger_entry', lang)),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final updatedEntry = PartyTransaction(
                  id: entry.id,
                  partyId: widget.party.id,
                  date: dateTypeNotifier.value == 'AD'
                ...
                final index = _ledger.indexWhere((e) => e.id == entry.id);
                if (index != -1) {
                  setState(() => _ledger[index] = updatedEntry);
  ...
  Future<void> _deleteEntry(PartyTransaction entry) async {
    final lang = languageNotifier.value;
    final confirm = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
    ...
    if (confirm == true) {
      setState(() => _ledger.removeWhere((e) => e.id == entry.id));
      await StorageService.savePartyLedger(widget.party.id, _ledger);
      _loadLedger();
    }
  }
  ...
  Widget _buildLedgerItem(
      dynamic e, double runningBalance, int index, String lang, String dateType,
      {bool isHeader = false, bool isOpening = false}) {
    String displayDate = '';
    if (!isHeader) {
      if (isOpening) {
        displayDate = '-';
    ...
    return TweenAnimationBuilder<double>(
      ...
      child: InkWell(
        onLongPress: isOpening || isHeader ? null : () => _showEntryOptions(e),
        child: Container(
              ...
              Expanded(
                  ...
                  child: Text(
                      isHeader
                          ? TranslationService.translate('credit', lang)
                          : (isOpening
                              ? (widget.party.type == BalanceType.cr
                                  ? widget.party.openingBalance.toString()
                                  : '0')
                              : (e.credit > 0 ? e.credit.toString() : '-')),
                      textAlign: TextAlign.right,
                      style: TextStyle(
                          color: isHeader ? null : Colors.red,
                          fontWeight:
                              isHeader ? FontWeight.bold : FontWeight.normal))),
              Expanded(
                  flex: 3,
            ...
            return Scaffold(
              appBar: AppBar(
                ...
                title: Row(
                  children: [
                    Hero(
                      tag: 'party-icon-${widget.party.id}',
              ...
              body: _loading
                  ...
                  : Column(
                      children: [
                        _buildLedgerItem(null, 0, 0, lang, dateType, isHeader: true),
                        Expanded(
                            ...
                            itemBuilder: (context, index) {
                              if (index == 0) {
                                return _buildLedgerItem(
                                    null, _runningBalances[0], 1, lang, dateType,
                                    isOpening: true);
                              }
                              return _buildLedgerItem(
                                  _ledger[index - 1], _runningBalances[index], index + 1, lang, dateType);
                            },
                          ),
                        ),
                      ],
